...
class XLSXWriter
{
	...
	protected function initializeSheet($sheet_name,array $sheet_options=array())
	{
		...
		$this->sheets[$sheet_name] = (object)array(
			...
			'finalized' => false,
		    //custom
		    'autofilter' => (isset($sheet_options['autofilter']))?$sheet_options['autofilter']:false,
		...
		if(!empty($sheet->freeze_pane)) {
    		...
    		$sheet->file_writer->write('<selection pane="bottomRight" sqref="'.$cell.'" activeCell="'.$cell.'"/>');
		}
		else {
	...
	public function writeSheetHeader($sheet_name, array $header_types, $suppress_row = false,array $sheet_options = array())
	{
		if (empty($sheet_name) || empty($header_types) || !empty($this->sheets[$sheet_name]))
			return;

		self::initializeSheet($sheet_name,$sheet_options);
		$sheet = &$this->sheets[$sheet_name];
		$sheet->columns = array();
	...
	public function writeSheetRow($sheet_name, array $row,array $sheet_options = array())
	{
	...
	public function writeSheet(array $data, $sheet_name='', array $header_types=array(), array $sheet_options=array())
	{
		$sheet_name = empty($sheet_name) ? 'Sheet1' : $sheet_name;
		$data = empty($data) ? array(array('')) : $data;
		if (!empty($header_types))
		{
			$this->writeSheetHeader($sheet_name, $header_types,false,$sheet_options);
		}
		foreach($data as $i=>$row)
		{
			$this->writeSheetRow($sheet_name, $row,$sheet_options);
		}
